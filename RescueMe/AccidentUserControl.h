...
// Global objects
__declspec(selectany)  cv::dnn::Net carMobileNetSSD = cv::dnn::readNetFromCaffe(
	"./dnn_models/MobileNetSSD/MobileNetSSD_deploy.prototxt", 
	"./dnn_models/MobileNetSSD/MobileNetSSD_deploy.caffemodel");

namespace RescueMe {

	/// <summary>
	/// Summary for AccidentUserControl
	/// </summary>
	public ref class AccidentUserControl : public System::Windows::Forms::UserControl
	{
	private:
...
/// </summary>
		void InitializeComponent(void)
		{
			...
			this->startButton->Click += gcnew System::EventHandler(this, &AccidentUserControl::startButton_Click);
			// 
			// sourceComboBox
			// 
			...
			this->thresholdNumericUpDown->ValueChanged += gcnew System::EventHandler(this, &AccidentUserControl::thresholdNumericUpDown_ValueChanged);
			// 
			// videoGroupBox
			// 
			...
			this->timer->Tick += gcnew System::EventHandler(this, &AccidentUserControl::timer_Tick);
			// 
			// openFileDialog
			// 
			...
			this->Name = L"AccidentUserControl";
...
#pragma endregion
	...
	private: System::Void timer_Tick(System::Object^  sender, System::EventArgs^  e) {
		...
		if (!frame.empty())
		{
			...
			carMobileNetSSD.setInput(inputBlob);
			cv::Mat detection = carMobileNetSSD.forward();
			...
			for (int i = 0; i < detectionMat.rows; i++)
			{
				...
				if (confidence > this->threshold && classId == this->carClassID)
				{
					int cols = frame.cols;
					int rows = frame.rows;

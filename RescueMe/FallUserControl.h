...
// Global objects
__declspec(selectany)  cv::dnn::Net personMobileNetSSD = cv::dnn::readNetFromCaffe(
...
namespace RescueMe {
	...
	/// <summary>
	/// Summary for FallUserControl
	/// </summary>
	public ref class FallUserControl : public System::Windows::Forms::UserControl
	{
	private:
	...
	private:
		...
		void InitializeComponent(void)
		{
			...
			this->startButton->Click += gcnew System::EventHandler(this, &FallUserControl::startButton_Click);
			// 
			// sourceComboBox
			// 
			...
			this->thresholdNumericUpDown->ValueChanged += gcnew System::EventHandler(this, &FallUserControl::thresholdNumericUpDown_ValueChanged);
			// 
			// videoGroupBox
			// 
			...
			this->timer->Tick += gcnew System::EventHandler(this, &FallUserControl::timer_Tick);
			// 
			// openFileDialog
			// 
		...
		private: System::Void timer_Tick(System::Object^  sender, System::EventArgs^  e) {
			...
			if (!frame.empty())
			{
				...
				personMobileNetSSD.setInput(inputBlob);
				cv::Mat detection = personMobileNetSSD.forward();
				...
				for (int i = 0; i < detectionMat.rows; i++)
				{
					...
					if (confidence > this->threshold && classId == this->personClassID)
					{
						int cols = frame.cols;
						int rows = frame.rows;
